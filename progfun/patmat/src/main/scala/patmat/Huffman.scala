...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
      def occurs(char: Char, count: List[(Char, Int)]): List[(Char, Int)] = {
        count match {
          case Nil => (char, 1) :: Nil
          case x :: xs => x match {
            case (theChar, theInt) => if (theChar == char) (char, theInt + 1) :: xs else x :: occurs(char, xs)
          }
        }
      }
    chars match {
      case Nil     => Nil
      case x :: xs => occurs(x, times(xs))
    }
  }
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = {
    trees match {
      case Nil          => Nil
      case x :: Nil     => x :: Nil
      case x :: y :: zs => (makeCodeTree(x, y) :: zs).sortWith((a, b) => weight(a) < weight(b))
    }
  }
   ...
   */
  def until[T](test: List[T] => Boolean, apply: List[T] => List[T])(input: List[T]): List[T] = {
    if (test(input)) input
    else until(test, apply)(apply(input))
  }
      ...
      def decodeHelper(subTree: CodeTree, bits: List[Bit]): List[Char] = {
        subTree match {
          ...
          case Fork(left, right, _, _) => bits match {
            case Nil => Nil
            case x :: xs => x match {
              case 0 => decodeHelper(left, xs)
              case 1 => decodeHelper(right, xs)
            }
          }
        }
      }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
      def encodeHelper(subTree: CodeTree)(text: List[Char]): List[Bit] = {
        text match {
          case Nil => Nil
          case x :: xs => subTree match {
            case Leaf(_, _)              => encodeHelper(tree)(xs)
            case Fork(left, right, _, _) => if (chars(left).contains(x)) 0 :: encodeHelper(left)(text) else 1 :: encodeHelper(right)(text)
          }
        }
      }
  ...
  def codeBits(table: CodeTable)(char: Char): List[Bit] = table match {
    case Nil => Nil
    case x :: xs => x match {
      case (ch, bits) => if (ch == char) bits else codeBits(xs)(char)
    }
  }
  ...
  def mergeCodeTables(a: CodeTable, b: CodeTable): CodeTable = {
      def prepend(table: CodeTable)(bit: Bit): CodeTable = {
        table match {
          case Nil => Nil
          case x :: xs => x match {
            case (ch, bits) => (ch, bit :: bits) :: prepend(xs)(bit)
          }
        }
      }
